CREATE TABLE appointment (
  ...
  CONSTRAINT ck_appointment_cancelled CHECK((status='CANCELLED' AND cancelled_at IS NOT NULL) OR (status='CONFIRMED' AND cancelled_at IS NULL))
);
...
CREATE TABLE appointment_idempotency (idempotency_key varchar(128) PRIMARY KEY CHECK(length(idempotency_key) >= 8), request_fingerprint char(64) NOT NULL, appointment_id uuid NOT NULL UNIQUE REFERENCES appointment(id), created_at timestamptz NOT NULL);
